...
class gui:
    ...
    def get_window(self, hu_img):
        if not self.slice.WindowCenter:
            self.window_center = (np.max(hu_img) + np.min(hu_img)) / 2
        else:
            self.window_center = self.slice.WindowCenter
        if not self.slice.WindowWidth:
            self.window_width = (np.max(hu_img) - np.min(hu_img))
        else:
            self.window_width = self.slice.WindowWidth
    ...
    def draw_canvas(self):
        ...
        self.label2.pack()
        self.label7 = tk.Label(self.label2, text='percent')
        self.label7.pack(side='left')
        self.input_percent = tk.Entry(self.label2, width=10)
        self.input_percent.insert(0, self.percent)
        self.input_percent.pack(side='left')
    ...
    def make_roi(self):
        self.percent = float(self.input_percent.get())
        self.window_center, self.window_width = self.roi_window(self.hu_img, percent=self.percent)
    ...
    def reset(self):
        self.window_center, self.window_width = self.get_window(self.hu_img)
        self.change_window(self.window_center, self.window_width)
